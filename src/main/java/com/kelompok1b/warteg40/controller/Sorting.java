        ...
        for (int gap = n / 2; gap > 0; gap /= 2) {
            for (int i = gap; i < n; i += 1) {
                ...
                switch (type) {
                    ...
                    case 1:
                        for (j = i; j >= gap
                                && array.get(j - gap).getItemName().compareTo(temp.getItemName()) > 0; j -= gap)
                            array.set(j, array.get(j - gap));
                        break;
                    case 2:
        ...
        for (int gap = n / 2; gap > 0; gap /= 2) {
            for (int i = gap; i < n; i += 1) {
                ...
                switch (type) {
                    case 0:
                        for (j = i; j >= gap
                                && array.get(j - gap).getIdTransaction() > temp.getIdTransaction(); j -= gap)
                            array.set(j, array.get(j - gap));
                        break;
                    case 1:
                        for (j = i; j >= gap
                                && array.get(j - gap).getCustomerName().compareTo(temp.getCustomerName()) > 0; j -= gap)
                            array.set(j, array.get(j - gap));
                        break;
                    case 2:
